...
def env_thread(args, thread_num, partition=True):
    ...
    if args.run_policy in ['custom', 'ppo2', 'walker']:
        ...
        if args.run_policy == 'ppo2':
            ...
            print('Expert model is obtained after {} step'.format(args.ppo2_timesteps))
        else:
    ...
    path_to_save = './srl_zoo/Ground_truth/{}'.format(args.env) + str(TIME_STAMP) + '.pickle'
    if not os.path.exists(path_to_save):
        os.system(r"touch {}".format(path_to_save))
    ...
    for i_episode in range(args.num_episode // args.num_cpu + 1 * (args.num_episode % args.num_cpu > thread_num)):
        ...
        while not done:
            ...
            ground_truth_position = env.getGroundTruth()
            ground_truth_per_episode.append(ground_truth_position)
        ...
        ground_truth_saver[i_episode] = ground_truth_per_episode
    print('shape',np.shape(env.getGroundTruth()))
    pickle.dump(ground_truth_saver, file_to_save)
    file_to_save.close()
