...
class Jaka:
    ...
    def gettargetjoints(self, target_pos):
        #target_pos = p.getLinkState(self.jaka_id, self.effector_id)[0]
    ...
    def apply_pos(self,pos,rest_poses):
        effector_id = self.effector_id
        
        joint_ranges = [4] * len(self.joints_key)
        target_velocity = [0] * len(self.joints_key)
        joint_poses = p.calculateInverseKinematics(self.jaka_id, effector_id, pos,
                                                       lowerLimits=self.joint_lower_limits,
                                                       upperLimits=self.joint_upper_limits,
                                                       jointRanges=joint_ranges,
                                                       restPoses=[0, 0, 1, 1, 1, 1]
                                                       )
        #joint_poses = p.calculateInverseKinematics(self.jaka_id, effector_id, pos)
        joints = [0,1,2,3,4,5]
        for joint, value in zip(joints, joint_poses):
            p.resetJointState(self.jaka_id, joint, value)
    ...
    def apply_action_pos_2(self, pos_commands):
        dx, dy, dz = pos_commands
        effector_id = self.effector_id
        link_pos = p.getLinkState(self.jaka_id, effector_id)
        self.effector_pos = link_pos[0]
...
if __name__ == '__main__':
    ...
    id1 = p.loadURDF("/urdf/simple_button.urdf", [-0.1, 0.4, 0])
    p.resetBasePositionAndOrientation(id1, [1, 1, 0],
                                          [0.000000, 0.000000, 0.000000, 1.000000])
    p.setGravity(0,0,-9.81)
    i = 0
    goal_conf = [-0.1, 0.4, 0]
    import ipdb
    #print(jaka.joint_lower_limits)
    #print(jaka.joint_upper_limits)

    while True:

        """start_conf = jaka.getGroundTruth()
        action = goal_conf - start_conf  
        d = np.linalg.norm(action, 2)
        jaka.apply_action_pos(action)
        if d < 0.1:
            print(p.getJointStates(jaka.jaka_id, [0,1,2,3,4,5])[0])
"""
        i += 1
        jaka.debugger_step()
        contact_obs = 0
        contact_obs += len(p.getContactPoints(jaka.jaka_id, planeId))
        for i in range(6):
            for j in range(6):
                #contact_obs += len(p.getContactPoints(bodyA=jaka.jaka_id, linkIndexA=i,bodyB=jaka.jaka_id,linkIndexB=j))
                if i!=j:
                    contact_obs += len(p.getClosestPoints(bodyA=jaka.jaka_id, bodyB=jaka.jaka_id, distance=0,
