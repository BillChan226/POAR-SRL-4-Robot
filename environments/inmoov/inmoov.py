...
from environments.inmoov.joints_registry import zhuaqu
...
class Inmoov:
    ...
    def reset_joints(self, joints_pos=None):
        ...
        if joints_pos is None:
            reset_pos = [0.] * len(self.joints_key)
            reset_pos[3] = 0.5
            for i, jointIndex in enumerate(self.joints_key):
                p.resetJointState(self.inmoov_id, jointIndex, reset_pos[i])
        else:
            assert len(joints_pos) == len(self.joints_key)
            for idx in range(len(joints_pos)):
                p.resetJointState(self.inmoov_id, self.joints_key[idx], joints_pos[idx])
    ...
    def zhua(self):
        for id in zhuaqu:
             p.resetJointState(self.inmoov_id, id, targetValue=zhuaqu[id])
    def song(self):
        for id in zhuaqu:
             p.resetJointState(self.inmoov_id, id, targetValue=0)
