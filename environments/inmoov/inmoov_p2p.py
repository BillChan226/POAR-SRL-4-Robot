...
class InmoovGymEnv(SRLGymEnv):
    ...
    def _reward(self):
        ...
        if distance < 0.1:
            re = 0
        for id in zhuaqu:
            re += len(p.getContactPoints(self._inmoov_id, self._tomato_id, id))
    ...
    def guided_step(self):
        ...
        eff_pos = p.getLinkState(self._inmoov_id, 37)[0]
        action = tomato_pos - eff_pos  
        print(np.linalg.norm(action, 2))
        self._inmoov.apply_action_pos(action)
        if np.linalg.norm(action, 2) < 0.1:
            self._inmoov.zhua()
