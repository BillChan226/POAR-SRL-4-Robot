...
### TODO: change button info to tomato
BUTTON_LINK_IDX = 1
...
class InmoovOneArmTomatoGymEnv(SRLGymEnv):
    """
    ...
    def __init__(self, urdf_root=pybullet_data.getDataPath(), renders=False, is_discrete=True, multi_view=False,
                 name="inmoov_onearm_tomato_gym", max_distance=0.8, action_repeat=1, shape_reward=False, action_joints=False,
        ...
        super(InmoovOneArmTomatoGymEnv, self).__init__(srl_model=srl_model,
    ...
    def getSRLState(self, observation):
        ...
        if self.srl_model in ["joints", "joints_position"]:
            ...
            state += list(self._inmoov.joint_positions)
        if len(state) != 0:
    ...
    def getArmPos(self):
        ...
        """
        return p.getLinkState(self._inmoov.inmoov_id, self._inmoov.effectorId)
    ...
    def reset(self):
        ...
        tomato2Id = p.loadURDF(os.path.join(self._urdf_sjtu, "tomato_plant.urdf"), [0.4, 0.4, 0.5],
        ...
        for _ in range(500):
            if self.action_joints:
                ### TODO:change
                self._inmoov.applyAction(list(np.array(self._kuka.joint_positions)[:7]) + [0, 0])
            else:
