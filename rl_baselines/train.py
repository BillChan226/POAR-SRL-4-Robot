...
def callback(_locals, _globals):
    ...
    if viz is None:
        viz = Visdom(port=VISDOM_PORT, env='GitExperiment')
        #viz = Visdom(port=VISDOM_PORT, env='Test')
...
def main():
    ...
    parser.add_argument('--gpu', type=str, default='1', help='The cpu we want to use')
    parser.add_argument('--demon-path', type=str, default='Undefined',
                        help='The path to store expert demonstration for domain resemblance loss in POAR')
    parser.add_argument('-sg', '--state-graph', action='store_true', default=False,
                        help='Whether to access state graph during training to diagnose the policy and learned states')
    ...
    assert not (ALGO_NAME == 'poar' and args.demon_path == 'Undefined'), \
        "Error: Demonstration path for POAR model is not defined"
