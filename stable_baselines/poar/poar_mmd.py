...
from stable_baselines.poar.utils import pca, mmd_loss
...
class POAR_MMD(ActorCriticRLModel):
    ...
    def __init__(
            ...
            split_dim=200, verbose=0, tensorboard_log='./tensorboard_logs', demo_path='Undefined', _init_setup_model=True, policy_kwargs=None,
            full_tensorboard_log=False, state_graph=1):

        super(
            POAR_MMD, self).__init__(policy=policy, env=env, verbose=verbose, requires_vec_env=True,
        ...
        if "domain" in self.srl_weight and self.srl_weight["domain"] > 0:
            ...
            demo_file = open(demonstration_path, 'rb')
            self.demonstration_dict = pickle.load(demo_file)
            self.demonstration = []
            for key in self.demonstration_dict.keys():
                self.demonstration.append(self.demonstration_dict[key])
            #print("SHAPE OF demonstration", np.shape(self.demonstration))
            self.ground_truth_dim = np.shape(self.demonstration)[2]
            self.demonstration = np.reshape(self.demonstration, (-1, self.ground_truth_dim))
            print("SHAPE OF demonstration", np.shape(self.demonstration))
            demo_file.close()
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    self.target_domain = tf.placeholder(tf.float32, [self.n_envs * self.n_steps // self.nminibatches, self.ground_truth_dim], name="target_domain_data")
                    ...
                    if "domain" in self.srl_weight and self.srl_weight["domain"] > 0:
                        weight_dict["domain_loss"] = self.srl_weight["domain"]
                        #self.target_domain = tf.constant([[2,5],[1,2]],tf.float32)
                        #train_model.srl_state = tf.constant([[2,5],[1,2]],tf.float32)
                        #print('SRLSTATE', train_model.srl_state)
                        dim_start = train_model.domain_dim[0]
                        dim_end = train_model.domain_dim[1]
                        domain_loss = mmd_loss(train_model.srl_state[..., dim_start:dim_end], self.target_domain)
                        srl_loss_dict["domain_loss"] = domain_loss
                    #tf.Print()
    ...
    def _train_step(
        ...
        #print("SHAPPPE", np.shape(obs))
        #print("type", obs)
        if "domain" in self.srl_weight and self.srl_weight["domain"] > 0:
            data_amount = self.n_envs * self.n_steps // self.nminibatches
            if data_amount > np.shape(self.demonstration)[0]:
                range_high = data_amount
            else:
                range_high = np.shape(self.demonstration)[0]
            index = random.sample(range(0, range_high), data_amount)
            target_domain_data_c = self.demonstration[index]
            td_map[self.target_domain] = target_domain_data_c
    ...
    def learn(self, total_timesteps, callback=None, seed=None, log_interval=1, tb_log_name="POAR", 
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in range(1, n_updates + 1):
                ...
                if self.plot_state_graph == 1:
                    ################ Plot the state graph every % updates ################
                    batch_latent.append(latent)
                    batch_reward.append([-1 if r < 0 else r for r in true_reward])

                    if update % self.save_frequency == 0:
                        #batch_latent_2 = batch_latent.copy()
                        latent = np.concatenate(batch_latent)
                        reward = np.concatenate(batch_reward)
                        ...
                        sc = plt.scatter(latent_pca[:,0], latent_pca[:,1], s=4, c = reward, cmap=plt.cm.get_cmap('Spectral_r'));
                        plt.colorbar(sc)
                        ...
                        plt.savefig(self.state_graph_path + "state_{}".format(update) + ".png")
                        ...
                        plt.close(fig)
                        if update % self.clear_batch_to_plot == 0:
...
class Runner(AbstractEnvRunner):
    ...
    def run(self):
        ...
        while iteration < self.n_steps:
            ...
            mb_obs.append(self.obs.copy())  # 每次添加num_cpu个n_steps (num_cpu=n_env)
